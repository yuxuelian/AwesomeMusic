...
public class CircleProgressBar extends View {
    ...
    private final RectF mProgressBgRectF = new RectF();
    ...
    private float mProgressDiffRadius;
    ...
    private void initFromAttributes(Context context, AttributeSet attrs) {
        ...
        mLineWidth = a.getDimensionPixelSize(R.styleable.CircleProgressBar_line_width, dip(context, DEFAULT_LINE_WIDTH));
        mProgressTextSize = a.getDimensionPixelSize(R.styleable.CircleProgressBar_progress_text_size, dip(context, DEFAULT_PROGRESS_TEXT_SIZE));
        mProgressStrokeWidth = a.getDimensionPixelSize(R.styleable.CircleProgressBar_progress_stroke_width, dip(context, DEFAULT_PROGRESS_STROKE_WIDTH));
        mProgressDiffRadius = a.getDimensionPixelSize(R.styleable.CircleProgressBar_progress_diff_radius, dip(context, DEFAULT_PROGRESS_STROKE_WIDTH));
    ...
    private void updateProgressShader() {
        if (mProgressStartColor != mProgressEndColor) {
            ...
            switch (mShader) {
                case LINEAR: {
                    shader = new LinearGradient(mProgressRectF.left,
                            mProgressRectF.top,
                            mProgressRectF.left,
                            mProgressRectF.bottom,
                            mProgressStartColor,
                            mProgressEndColor, Shader.TileMode.CLAMP);
                ...
                case SWEEP: {
                    ...
                    float rotateDegrees = (float) (-(mCap == Paint.Cap.BUTT && mStyle == SOLID_LINE ? 0 : Math.toDegrees(radian)));
    ...
    private void drawSolidLineProgress(Canvas canvas) {
        if (mDrawBackgroundOutsideProgress) {
            float startAngle = MAX_DEGREE * mProgress / mMax;
            float sweepAngle = MAX_DEGREE - startAngle;
            canvas.drawArc(mProgressBgRectF, startAngle, sweepAngle, false, mProgressBackgroundPaint);
        } else {
            canvas.drawArc(mProgressBgRectF, 0.0f, MAX_DEGREE, false, mProgressBackgroundPaint);
        }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mProgressBgRectF.top = mCenterY - mRadius;
        mProgressBgRectF.bottom = mCenterY + mRadius;
        mProgressBgRectF.left = mCenterX - mRadius;
        mProgressBgRectF.right = mCenterX + mRadius;

        mProgressRectF.top = mProgressBgRectF.top + mProgressDiffRadius;
        mProgressRectF.bottom = mProgressBgRectF.bottom - mProgressDiffRadius;
        mProgressRectF.left = mProgressBgRectF.left + mProgressDiffRadius;
        mProgressRectF.right = mProgressBgRectF.right - mProgressDiffRadius;

        updateProgressShader();

        //Prevent the progress from clipping
        mProgressBgRectF.inset(mProgressStrokeWidth / 2, mProgressStrokeWidth / 2);
